...
jieba.load_userdict("C:\\Users\\Jiuning\\PycharmProjects\\CNEpistemicCluster\\jieba\\userdict.txt")
...
def stop_words(stop_word_file):
    ...
    result = jieba.cut(words, cut_all=False)
    new_words = []
    for r in result:
        new_words.append(r)
...
def get_all_vector(file_path,stop_words_set):
    ...
    posts = [ open(name,"r", encoding='utf-8').read() for name in names ]
    docs = []
    word_set = set()
    for post in posts:
...
if __name__ == "__main__":
    d = os.getcwd() # path.dirname(__file__) if "__file__" in locals() else os.getcwd()
    zn_StopWordPath = path.join(d, 'stopwords.txt')
    stop_words = stop_words(zn_StopWordPath)
    # stop_words = stop_words('C:\\Users\\Jiuning\\PycharmProjects\\CNEpistemicCluster\\jieba\\chinese\\stopwords.txt')

    names, tfidf_mat = get_all_vector("C:\\Users\\Jiuning\\PycharmProjects\\CNEpistemicCluster\\jieba\\chinese\\",stop_words)
